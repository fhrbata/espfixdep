...
int membuf_reinit(struct membuf *mb, size_t s)
{
	if (s <= membuf_size(mb)) {
		int allocated = membuf_is_allocated(mb);
		mb->size = s;
		if (allocated)
...
int membuf_fread(struct membuf *mb, FILE *fp)
{
	long size;
...
int membuf_fwrite(struct membuf *mb, FILE *fp)
{
...
int membuf_cat_mbs(struct membuf *dst, struct membuf **src, size_t src_cnt)
{
