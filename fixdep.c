...
#define DATA_SIZE (1024 * 7)
	...
	if (membuf_fread(&depfile.data, fp)) {
		err("cannot read '%s'", fn);
		goto err;
	}

	char *beg = membuf_buf(&depfile.data);
	char *end = membuf_end(&depfile.data);
...
#define DATA_SIZE (1024 * 10 * 3)
	...
	if (membuf_fread(&config.data, fp)) {
		err("cannot read '%s'", fn);
		goto err;
	}

	char *c = membuf_buf(&config.data);
	char *end = membuf_end(&config.data);
...
#define DEP_FN_SIZE (1024)
	...
	DEFINE_MEMBUF(dep, dep_fn, DEP_FN_SIZE);
	...
	if (membuf_fwrite(&depfile->target, fp))
		goto err;

	if (membuf_fwrite(&semi, fp))
		goto err;

	for (int i = 0; i < depfile->deps_cnt; i++) {
		if (membuf_fwrite(&sep, fp))
			goto err;
		if (membuf_fwrite(&deps[i], fp))
			goto err;
	}
	...
	for (int i = 0; i < config->opts_cnt; i++) {
		...
		if (membuf_empty(&depfile->sdkconfig_dir)) {
			if (membuf_cat(&dep, &opts[i], &ext))
				goto err;
		} else {
			if (membuf_cat(&dep, &depfile->sdkconfig_dir, &slash,
				       &opts[i], &ext))
				goto err;
		}
		...
		if (membuf_fwrite(&dep, fp))
			goto err;
	}

	if (membuf_fwrite(&eol, fp))
		goto err;
