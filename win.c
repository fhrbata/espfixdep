...
#define CMDL_WC_CNT (1024 * 10)
	...
	if (membuf_reinit(&cmdl, (wcslen(c) + 1) * sizeof(wchar_t)))
		goto err;
...
int mbs_to_wcs(const char *mbs, struct membuf *wcs)
{
	int rv;
	DWORD err;
...
int wcs_to_mbs(const wchar_t *wcs, struct membuf *mbs)
{
	int rv;
	DWORD err;
...
#define WMSG_WC_CNT (1024 * 2)
	...
	if (n >= membuf_size(&msg)) {
		if (membuf_reinit(&msg, n + 1))
			goto err;

		va_copy(args_copy, args);
		vsnprintf(membuf_buf(&msg), membuf_size(&msg), fmt, args);
		va_end(args_copy);
	}

	if (!GetConsoleMode(h, &dwMode)) {
		rv = fwrite(membuf_buf(&msg), 1, membuf_size(&msg) - 1, stream);
		goto err;
	}

	int wmsg_size = mbs_to_wcs(membuf_buf(&msg), &wmsg);
	if (!wmsg_size)
		goto err;
...
#define ARGV_BUF_SIZE (1024)
	...
	if (membuf_reinit(&argv_mb, argc + 1)) {
		err_errno("argv malloc failed");
		goto err;
	}

	if (membuf_reinit(&args_mb, (wcslen(GetCommandLineW()) + 1) * 4)) {
	...
	int remaining = membuf_size(&args_mb);
	int offset = 0;
	int cnt = 0;
